...
export class ProductoListComponent implements OnInit {
  ...
  guardar() {
    const nombreValido = /^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{1,20}$/.test(this.newProduct.nombre.trim());
    const descripcionValida = /^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{1,100}$/.test(this.newProduct.descripcion.trim());
    if (!nombreValido) {
      this.mostrarMensaje('El nombre solo puede contener letras y máximo 20 caracteres.', 'error');
      return;
    }
    if (!descripcionValida) {
      this.mostrarMensaje('La descripción solo puede contener letras y máximo 100 caracteres.', 'error');
      return;
    }
    // Generar código interno automáticamente si es nuevo
    if (!this.editando) {
      this.newProduct.codigo = 'PROD-' + Math.floor(Math.random() * 10000).toString().padStart(3, '0');
    }
    const payload = {
